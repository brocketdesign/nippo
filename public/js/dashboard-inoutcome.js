$(document).ready(async function () {
    ...
    function inputInit() {
        ...
        if (document.querySelector('#inoutcome-filtered-data')) {
            initTable($('#inoutcome-filtered-data'))
        }
    }
    ...
    function addElement(formSelect) {
        let formID = '.form-container.' + formSelect
    ...
    function updateTableOnResponse(data) {
        let tbody = $('#inoutcome-filtered-data')
        ...
        data.forEach(function(item) {
            ...
            let genba = item.現場名
            let bikou = item.備考
            let price = parseInt(item.査定金額)
            let zeiritu = parseInt(item.税率)
            let tax = (price * zeiritu / 100).toFixed()
    ...
    function requestFilter(query, callback) {
        $('#searching-progress').fadeIn(500)
        $.post('/api/inoutcome/filter/', query, function (data) {
            // console.log(data)
    ...
    function saveToDB(formSelect, callback) {
        ...
        $.post('/api/inoutcome/daityou/', result, function (data) {
            console.log(data)
            setTimeout(() => {
